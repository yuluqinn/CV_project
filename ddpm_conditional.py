...
class Diffusion:
    ...
    def load(self, model_cpkt_path, model_ckpt="model.pt"):
        #self.model.load_state_dict(torch.load(os.path.join(model_cpkt_path, model_ckpt)))
        #self.ema_model.load_state_dict(torch.load(os.path.join(model_cpkt_path, ema_model_ckpt)))
        checkpoint = torch.load((os.path.join(model_cpkt_path, model_ckpt)))
        self.model.load_state_dict(checkpoint['model_state_dict'])
        self.ema_model.load_state_dict(checkpoint['ema_model_state_dict'])
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['start_epoch']
        self.mse = checkpoint['loss']
        return start_epoch

    def save_model(self, run_name, epochs, use_wandb=False):
       ...
       # torch.save(self.model.state_dict(), os.path.join("models", run_name, f"ckpt.pt"))
        ...
        if use_wandb:
            at = wandb.Artifact("model", type="model", description="Model weights for DDPM conditional", metadata={"epoch": epochs})
    ...
    def fit(self, args, start_epoch):
       # for epoch in progress_bar(range(args.epochs), total=args.epochs, leave=True):
        ...
        # save model
        self.save_model(run_name=args.run_name, epochs=args.epochs, use_wandb=args.use_wandb)
...
if __name__ == '__main__':
    ...
    with wandb.init(project="train_sd", group="train", config=config) if config.use_wandb else nullcontext():
        start_epoch = diffuser.prepare(config)
        diffuser.fit(config, start_epoch)
